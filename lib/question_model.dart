...
List<Question> getQuestions() {
  List<Question> list = [
    Question(
      id: 'KSH-0',
      questionText:
          '괄호( ? ) 안에 알맞은 단어는 무엇인가?\n\n기준금리에 신용도 등의 차이에 따라 달리 덧붙이는 금리를 ( ? )(또는 스프레드, spread)(이)라고 한다. 예를 들어 은행이 대출금리를 결정할 때 고객의 신용위험에 따라 조달금리에 추가하는 금리를 말한다. 한편 만기가 길어지면 추가로 가산되는 금리를 기간 ( ? )(텀스프레드, term spread)(이)라고 하는데 이것도 일종의 스프레드이다. 통상 신용도가 높으면 ( ? )(이)가 낮고, 신용도가 낮으면 ( ? ) 즉 스프레드는 커진다. 한편 채권시장에서는 비교대상이 되는 금융상품의 기준금리에 대비한 차이를 스프레드라고 한다. 보통 해외에서 채권을 발행할 때 동일한 만기의 미국 국채나 리보(LIBOR, 런던은행간금리)가 기준금리가 되고 여기에 신용도 등에 따라 ( ? )(이)가 붙어서 발행되는 게 보통이다. 실제로 우리나라는 지난 1997년 외환위기 당시에 해외에서 기채할 때 높은 ( ? )을(를) 지불한 경험이 있다. 기준금리와의 차이를 나타내는 ( ? ) 또는 스프레드는 보통 베이시스 포인트(bp, basis point)로 나타내는데 예를 들면 0.5%의 금리격차를 50bp로 표기하고 1%는 100bp로 표시된다.',
      answerList: [
        Answer('가산금리', true),
        Answer('기준금리', false),
        Answer('고금리', false),
    ...
    Question(
      id: 'KSH-1',
      questionText:
          '괄호( ? ) 안에 알맞은 단어는 무엇인가?\n\n( ? )은(는) 경기 종합지수와는 달리 경기변동의 진폭이나 속도는 측정하지 않고 변화 방향만을 파악하는 것으로서 경기의 국면 및 전환점을 식별하기 위한 지표로서 경기확산지수(diffusion index)라고도 한다. ( ? )은(는) 경기변동이 경제의 특정 부문으로부터 전체 경제로 확산, 파급되는 과정을 경제부문을 대표하는 각 지표들을 통하여 파악하기 위한 지표이다. 이때 경제지표 간의 연관관계는 고려하지 않고 변동 방향만을 종합하여 지수로 만든다. 경기 종합지수와 같이 선행, 동행, 후행지수로 작성되며, 계절변동과 불규칙 요인이 제거된 전체 계열 중에서 전월에 비해 증가한 지표수가 차지하는 비중으로 나타낸다. 예컨대 10개의 대표 계열 중 7개의 지표가 증가하는 방향으로 움직였다면 경기확산지수는 70%로 나타난다. 만약 ( ? )이(가) 기준선인 50보다 크면 경기는 확장국면에, 50보다 작으면 수축국면에 있는 것으로 판단한다.',
      answerList: [
