...
List<Question> getQuestions() {
  List<Question> list = [
    Question(
      id: 'KSH-0',
      questionText:
          '괄호( ? ) 안에 들어갈 단어는 무엇인가?\n\n기준금리에 신용도 등의 차이에 따라 달리 덧붙이는 금리를 ( ? )(또는 스프레드, spread)라고 한다. 예를 들어 은행이 대출금리를 결정할 때 고객의 신용위험에 따라 조달금리에 추가하는 금리를 말한다. 한편 만기가 길어지면 추가로 가산되는 금리를 기간 ( ? )(텀스프레드, term spread)라고 하는데 이것도 일종의 스프레드이다. 통상 신용도가 높으면 ( ? ) 낮고, 신용도가 낮으면 ( ? ) 즉 스프레드는 커진다. 한편 채권시장에서는 비교대상이 되는 금융상품의 기준금리에 대비한 차이를 스프레드라고 한다. 보통 해외에서 채권을 발행할 때 동일한 만기의 미국 국채나 리보(LIBOR, 런던은행간금리)가 기준금리가 되고 여기에 신용도 등에 따라 ( ? ) 붙어서 발행되는 게 보통이다. 실제로 우리나라는 지난 1997년 외환위기 당시에 해외에서 기채할 때 높은 ( ? )를 지불한 경험이 있다. 기준금리와의 차이를 나타내는 ( ? ) 또는 스프레드는 보통 베이시스 포인트(bp, basis point)로 나타내는데 예를 들면 0.5%의 금리격차를 50bp로 표기하고 1%는 100bp로 표시된다.',
      answerList: [
        Answer('가산금리', true),
        Answer('기준금리', false),
        Answer('고금리', false),
